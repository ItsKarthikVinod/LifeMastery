  ...
  return (
    <>
      {isVisible ? (
        <Draggable handle=".drag-handle">
          <div
            className={`pomodoro-widget fixed ${
              isMaximized ? 'top-0 left-0 w-full h-full bg-gradient-to-r from-purple-600 to-blue-500' : 'bottom-10 right-10'
        ...
        <button
          onClick={toggleVisibility}
          className="fixed bottom-4 right-4 bg-teal-600 text-white px-4 py-2 rounded-full shadow-lg z-50 hover:bg-teal-700 transition duration-300"
        >
          Show Pomodoro
        </button>
      )}
